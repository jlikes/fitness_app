...
function calcCalWithGoal(person) {
    if (person.goal === "GAIN")
        return person.tdee + 500;
    if (person.goal === "LOSE")
        return person.tdee - 200;
    else
        return person.tdee;
}
function calcProteinWithGoal(person, cals) {
    if (person.goal === "LOSE")
        return Math.floor((cals * .4) / 4);
    else
        return Math.floor((cals * .3) / 4);
}
function calcFatsWithGoal(person, cals) {
    if (person.goal === "LOSE")
        return Math.floor((cals * .2) / 4);
    else
        return Math.floor((cals * .3) / 4);
}
